...
// ── Stat card ─────────────────────────────────────────────────────────────────
const StatCard = ({ label, value, color, bg, emoji, onClick, pulse }) => (
  <button
    onClick={onClick}
    className={`card flex items-center gap-4 p-5 text-left w-full transition-all hover:shadow-md hover:-translate-y-0.5 ${pulse ? 'ring-2 ring-amber-400 ring-offset-1' : ''}`}
  >
...
const SeniorEmployeeDashboard = () => {
  ...
  const handleAction = async (note) => {
    ...
    try {
      ...
      toast.success(
        type === 'approve' ? '✅ Approved & forwarded to Department Manager!' :
        type === 'reject'  ? '❌ Requirement rejected.' :
        '↩️ Returned to employee for correction.'
      );
      setModal(null);
      loadStats(); loadQueue();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card overflow-hidden">
        <div className="bg-gradient-to-r from-slate-800 to-slate-600 px-6 py-7 sm:px-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-300">{user?.role} · {user?.department?.departmentName || '—'}</p>
            </div>
      ...
      {!loadingStats && pending > 0 && (
        <div className="flex items-center gap-3 rounded-xl border border-amber-300 bg-amber-50 px-5 py-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-700">Review and approve, return, or reject from the queue below.</p>
          </div>
      ...
      {/* Quotation Pending alert */}
      {!loadingStats && quotPending > 0 && (
        <div className="flex items-center gap-3 rounded-xl border border-cyan-300 bg-cyan-50 px-5 py-3">
          <span className="text-2xl animate-bounce">📁</span>
          <div className="flex-1">
            <p className="text-sm font-semibold text-cyan-800">{quotPending} request{quotPending > 1 ? 's' : ''} require quotation upload</p>
            <p className="text-xs text-cyan-700">The Department Director has approved these. Please upload vendor quotations and submit to the Department Manager.</p>
          </div>
          <button onClick={() => navigate('/review/queue')} className="shrink-0 rounded-lg bg-cyan-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-cyan-700">Upload Now →</button>
        </div>
      )}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {[
            ...
            { emoji:'📁', label:'Upload Quotations',  onClick: () => navigate('/review/queue') },
            { emoji:'✅', label:'Approved Items',     onClick: () => navigate('/review/queue') },
            { emoji:'📊', label:'All Requirements',   onClick: () => navigate('/review/queue') },
            { emoji:'👤', label:'My Profile',         onClick: () => navigate('/profile') },
            { emoji:'🏠', label:'Dashboard',          onClick: () => navigate('/dashboard/senior-employee') },
          ].map(a => (
      ...
      <div className="card p-6">
        ...
        <ol className="relative border-l-2 border-slate-200 pl-8 space-y-5">
          {[
            ...
            { label: 'Senior Employee ← You', desc: 'Initial review, quotation upload, PO send, GRN, invoice submission', done: false, active: true  },
            { label: 'Department Manager',  desc: '≤ AED 500 → quotation stage | > AED 500 → Budget Controller',    done: false, active: false },
            { label: 'Budget Controller',   desc: 'AED 500–3,000 → Dept Head | > AED 3,000 → MD',                  done: false, active: false },
            { label: 'Managing Director',   desc: 'Executive approval (> AED 3,000)',                               done: false, active: false },
            { label: 'Department Head',     desc: 'Sign-off + PO digital signature + GRN final approval',           done: false, active: false },
            { label: 'Senior Accountant',   desc: 'Three-way match: PO + GRN + Invoice → payment approval',         done: false, active: false },
          ].map((s, i) => (
            <li key={i} className="relative">
              <span className={`absolute -left-[1.35rem] top-0.5 flex h-6 w-6 items-center justify-center rounded-full border-2 text-xs font-bold
                ${s.done ? 'border-emerald-500 bg-emerald-500 text-white' : s.active ? 'border-navy-600 bg-navy-700 text-white ring-4 ring-navy-100' : 'border-slate-300 bg-white text-slate-400'}`}>
                {s.done ? '✓' : i + 1}
              </span>
              <p className={`text-sm font-semibold ${s.active ? 'text-navy-700' : s.done ? 'text-emerald-700' : 'text-slate-400'}`}>{s.label}</p>
...
export default SeniorEmployeeDashboard;
