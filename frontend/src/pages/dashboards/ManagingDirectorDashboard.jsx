...
const PIE_COLORS = { 'MD Review':'#e11d48','Budget Check':'#8b5cf6','Under Review':'#f59e0b',Approved:'#10b981',Rejected:'#ef4444',Returned:'#f97316',Completed:'#7c3aed',Draft:'#94a3b8' };

const StatCard = ({ label, value, color, bg, emoji, onClick, pulse, sub }) => (
  <button onClick={onClick} className={`card flex items-center gap-4 p-5 text-left w-full transition-all hover:shadow-md hover:-translate-y-0.5 ${pulse?'ring-2 ring-rose-400 ring-offset-1':''}`}>
...
const ManagingDirectorDashboard = () => {
  ...
  const handleAction = async (note) => {
    ...
    try {
      ...
      toast.success(
        type === 'approve'
          ? `✅ Approved by MD (AED ${(req.estimatedTotalPrice||0).toLocaleString()}) — Forwarded to Department Head.`
          : type === 'reject'
          ? '❌ Rejected by MD. Requirement rejected.'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-rose-200 bg-rose-50 px-5 py-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-rose-800">Managing Director — Executive Approval</p>
          <p className="text-xs text-rose-700 mt-0.5">
            You receive requirements where the total <strong>exceeds AED 3,000</strong>. These have been reviewed by SE, DM, and Budget Controller.
            Your approval forwards the request to the <strong>Department Head</strong> for sign-off before the quotation stage begins.
          </p>
        </div>
      </div>
      ...
      {!loadingStats && pending > 0 && (
        <div className="flex items-center gap-3 rounded-xl border border-rose-300 bg-rose-50 px-5 py-3">
          <span className="text-2xl animate-bounce">🏛️</span>
          <div className="flex-1">
            <p className="text-sm font-semibold text-rose-800">
              {pending} high-value request{pending > 1 ? 's' : ''} require your executive approval
            </p>
            <p className="text-xs text-rose-700">
              Total value pending: <strong>AED {totalPendingValue.toLocaleString()}</strong>
            </p>
          </div>
          <button onClick={() => navigate('/review/queue')} className="shrink-0 rounded-lg bg-rose-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-rose-700">
            Review Now →
          </button>
        </div>
      )}
      ...
      <div>
        ...
        {loadingStats ? (
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <StatCard emoji="🏛️" label="MD Pending"   value={pending}                        color="text-rose-600"    bg="bg-rose-50"    onClick={()=>navigate('/review/queue')} pulse={pending>0} sub={`AED ${totalPendingValue.toLocaleString()} total`} />
            <StatCard emoji="✅" label="Total Approved" value={stats?.stats?.approved??0}     color="text-emerald-600" bg="bg-emerald-50" onClick={()=>navigate('/review/queue')} />
            <StatCard emoji="❌" label="Total Rejected" value={stats?.stats?.rejected??0}     color="text-red-600"     bg="bg-red-50"     onClick={()=>navigate('/review/queue')} />
            <StatCard emoji="💰" label="Budget Check"   value={stats?.stats?.budgetCheck??0}  color="text-violet-600"  bg="bg-violet-50"  onClick={()=>navigate('/review/queue')} sub="With Budget Controller" />
          </div>
        )}
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {[
            { emoji:'🏛️', label:'MD Review Queue',  onClick:()=>navigate('/review/queue') },
            { emoji:'✅', label:'Approved',          onClick:()=>navigate('/review/queue') },
            { emoji:'📊', label:'All Org Requests',  onClick:()=>navigate('/review/queue') },
            { emoji:'💰', label:'Budget Overview',   onClick:()=>navigate('/review/queue') },
            { emoji:'👤', label:'My Profile',        onClick:()=>navigate('/profile')      },
            { emoji:'🏠', label:'Dashboard',         onClick:()=>navigate('/dashboard/managing-director') },
          ].map(a=>(
      ...
      <div className="card overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div>
            <h3 className="text-sm font-semibold text-slate-700">MD Review Queue</h3>
            <p className="text-xs text-slate-400 mt-0.5">Very high-value requests (&gt; ₹5,000) escalated from Budget Controller</p>
          </div>
        ...
        {loadingQueue ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h4 className="text-base font-semibold text-slate-700">No executive reviews pending</h4>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50 text-left">
                  {['Req. Number','Employee','Department','Item','Priority','Est. Amount','Date','Actions'].map(h=>(
              ...
              <tbody className="divide-y divide-slate-100">
                {queue.map(r => (
                  <tr key={r._id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-slate-600 text-xs">{r.departmentName || r.department?.departmentName}</td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap font-bold text-rose-700">AED {(r.estimatedTotalPrice||0).toLocaleString()}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-xs text-slate-500">{new Date(r.createdAt).toLocaleDateString()}</td>
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="flex items-center gap-1.5">
                        <button onClick={()=>navigate(`/review/${r._id}`)} className="rounded-md bg-rose-50 px-2.5 py-1 text-xs font-semibold text-rose-700 hover:bg-rose-100">Review</button>
      ...
      <div className="card p-6">
        <h3 className="mb-5 text-sm font-semibold text-slate-700">Full Approval Workflow — Your Position</h3>
        <ol className="relative border-l-2 border-slate-200 pl-8 space-y-4">
          {[
            { label:'Requesting Employee', desc:'Creates & submits requirement',               done:true,  active:false },
            { label:'Senior Employee',     desc:'Initial review & endorsement',                done:true,  active:false },
            { label:'Department Manager',  desc:'Dept-level approval (≤ AED 500 → quotation stage | > AED 500 → BC)', done:true,  active:false },
            { label:'Budget Controller',   desc:'Budget review (AED 500–AED 3,000 → Dept Head | > AED 3,000 → MD)', done:true,  active:false },
            { label:'Managing Director ← You', desc:'Executive approval (> AED 3,000) → forwards to Department Head', done:false, active:true },
            { label:'Department Head',     desc:'Sign-off → SE collects quotations',           done:false, active:false },
            { label:'PO → GRN → Payment',  desc:'Quotation / PO / Goods Receipt / Invoice stages', done:false, active:false },
            { label:'Senior Accountant',   desc:'Three-way match → payment approved',          done:false, active:false },
          ].map((s,i)=>(
            <li key={i} className="relative">
              <span className={`absolute -left-[1.35rem] top-0.5 flex h-6 w-6 items-center justify-center rounded-full border-2 text-xs font-bold
                ${s.done?'border-emerald-500 bg-emerald-500 text-white':s.active?'border-rose-600 bg-rose-700 text-white ring-4 ring-rose-100':'border-slate-300 bg-white text-slate-400'}`}>
                {s.done?'✓':i+1}
              </span>
              <p className={`text-sm font-semibold ${s.active?'text-rose-700':s.done?'text-emerald-700':'text-slate-400'}`}>{s.label}</p>
